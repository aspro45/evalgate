...
def _norm_review(raw) -> dict:
    ...
    outcome = _s(data.get("outcome", data.get("decision", "conditional")), 40).lower()
    if outcome in ("true", "yes", "support", "supports", "approved", "valid", "confirmed", "affirmed", "upheld", "bloomed", "settled"):
        outcome = "approved"
    ...
    support = _bounded_int(data.get("supportBps", 8200 if outcome == "approved" else 2600), 0, 10000, 5000)
    ...
    if summary == "":
        summary = "EvalGate review outcome: " + outcome
    if rationale == "":
        rationale = summary
...
def _review_prompt(protocol: str, case: dict, evidence_text: str) -> str:
    return (
        "You are EvalGate V2. Independently evaluate the bounded public evidence.\n" + SECURITY +
...
def _ruling_prompt(mode: str, case: dict, filing: dict, evidence_text: str) -> str:
    return (
        "You are EvalGate V2 resolving a " + mode + " filing.\n" + SECURITY +
        "\nCase JSON: " + json.dumps(case, sort_keys=True) +
...
class EvalGate(gl.Contract):
    ...
    evaluations: DynArray[str]
    risk_findings: DynArray[str]
    idx_case_evaluations: TreeMap[str, str]
    idx_case_risk_findings: TreeMap[str, str]
    ...
    def __init__(self) -> None:
        ...
        self.protocol = "EvalGate governs model releases, model cards, evaluation suites, benchmark runs, risk findings and release gates. Every decision requires public sources, prompt-injection resistance, comparative validator reasoning, operator permissions, challenge rights, appeal rights and a visible audit trail."
    ...
    def _create_case(self, kind: str, title: str, claim: str, source_url: str, fields: dict) -> str:
        ...
        case = {"id": cid, "kind": _s(kind, 60), "actor": actor, "title": _s(title, 260),
                ...
                "status": "DRAFT", "outcome": "pending", "confidenceBps": 0, "supportBps": 0, "contradictionBps": 0,
        ...
        self._idx_add(self.idx_status, "DRAFT", cid)
        self._idx_add(self.idx_actor, actor.lower(), cid)
        self.recent_ids.append(cid)
        self._audit(case, "open", "case opened", "", "DRAFT")
    ...
    def add_evidence(self, case_id: str, url: str, title: str, note: str) -> str:
        ...
        if case["status"] in ("GATED", "ARCHIVED"):
    ...
    @gl.public.write
    def add_evaluation_run(self, case_id: str, label: str, reference: str, source_url: str) -> str:
        self.clock += 1
        case = self._load_case(case_id)
        self._require_operator(case)
        if case["status"] in ("GATED", "ARCHIVED"):
            raise Exception("case_closed")
        item_id = str(len(self.evaluations))
        row = {"id": item_id, "caseId": case["id"], "actor": self._actor(),
               "type": "evaluation", "label": _s(label, 260), "reference": _s(reference, 500),
               "sourceUrl": _clean_url(source_url), "createdAt": str(int(self.clock))}
        self.evaluations.append(json.dumps(row))
        self._idx_add(self.idx_case_evaluations, case["id"], item_id)
        self._audit(case, "add_evaluation", label, case["status"], case["status"])
        self._store_case(case)
        self._rep_bump(self._actor(), "evidence", 35)
        return item_id

    @gl.public.write
    def add_risk_finding(self, case_id: str, label: str, reference: str, source_url: str) -> str:
        self.clock += 1
        case = self._load_case(case_id)
        self._require_operator(case)
        if case["status"] in ("GATED", "ARCHIVED"):
            raise Exception("case_closed")
        item_id = str(len(self.risk_findings))
        row = {"id": item_id, "caseId": case["id"], "actor": self._actor(),
               "type": "risk_finding", "label": _s(label, 260), "reference": _s(reference, 500),
               "sourceUrl": _clean_url(source_url), "createdAt": str(int(self.clock))}
        self.risk_findings.append(json.dumps(row))
        self._idx_add(self.idx_case_risk_findings, case["id"], item_id)
        self._audit(case, "add_risk_finding", label, case["status"], case["status"])
        self._store_case(case)
        self._rep_bump(self._actor(), "evidence", 35)
        return item_id

    @gl.public.view
    def get_evaluations(self, case_id: str) -> str:
        return self._rows(self.evaluations, self._ilist(self.idx_case_evaluations, case_id))

    @gl.public.view
    def get_risk_findings(self, case_id: str) -> str:
        return self._rows(self.risk_findings, self._ilist(self.idx_case_risk_findings, case_id))
    ...
    def review_with_genlayer(self, case_id: str) -> str:
        ...
        if case["status"] in ("GATED", "ARCHIVED"):
            raise Exception("case_closed")
        before = case["status"]
        self._set_status(case, "EVALUATING")
        ...
        res = json.loads(gl.eq_principle.prompt_comparative(leader, "Equal when both assessments reach the same directional conclusion: support, contradiction, or indeterminate. Score differences are immaterial within the same conclusion band."))
        if int(res["supportBps"]) >= int(res["contradictionBps"]) + 1500:
            ...
            res["outcome"] = "conditional"
            res["confidenceBps"], res["supportBps"], res["contradictionBps"] = 6000, 5000, 5000
        res["summary"] = "EvalGate review resolved " + res["outcome"] + " from " + str(len(case["evidenceIds"]) + 1) + " independently fetched sources."
        ...
        self._audit(case, "review", res["summary"], before, "CHALLENGE_WINDOW")
        self._store_case(case)
    ...
    def open_challenge_window(self, case_id: str) -> None:
        ...
        self._require_operator(case)
        before = case["status"]
        if before == "CHALLENGE_WINDOW" and _now() <= int(case.get("challengeDeadline", "0")):
            return
        if before != "REVIEWED":
    ...
    def submit_challenge(self, case_id: str, claim: str, evidence_url: str) -> str:
        self.clock += 1
        case = self._load_case(case_id)
        if case["status"] != "CHALLENGE_WINDOW" or _now() > int(case.get("challengeDeadline", "0")):
    ...
    def resolve_challenge_with_genlayer(self, case_id: str, challenge_id: str) -> None:
        ...
        res["revisedOutcome"] = "blocked" if res["ruling"] == "accepted" else "conditional" if res["ruling"] == "partially_accepted" else case["outcome"]
        res["reason"] = "EvalGate challenge received " + str(merit) + " bps independent validator merit."
        ...
        case["summary"] = "EvalGate challenge " + res["ruling"] + "; canonical outcome " + case["outcome"] + "."
    ...
    def resolve_appeal_with_genlayer(self, case_id: str, appeal_id: str) -> None:
        ...
        if res["ruling"] == "granted":
            res["revisedOutcome"] = "approved" if int(res["confidenceDeltaBps"]) >= 0 else "blocked"
        elif res["ruling"] == "partially_granted":
            res["revisedOutcome"] = "conditional"
        else:
            res["revisedOutcome"] = case["outcome"]
        res["reason"] = "EvalGate appeal received " + str(merit) + " bps independent validator merit."
        ...
        case["summary"] = "EvalGate appeal " + res["ruling"] + "; canonical outcome " + case["outcome"] + "."
        case["rationale"] = res["reason"]
        case["riskFlags"] = _flags(case.get("riskFlags", []) + res["riskFlags"])
    ...
    def finalize_case(self, case_id: str) -> None:
        ...
        if _now() < maturity:
            raise Exception("challenge_period_active")
        before = case["status"]
        self._set_status(case, "GATED")
        self._audit(case, "finalize", "case finalized after challenge and appeal maturity", before, "GATED")
        self._store_case(case)
    ...
    def get_contract_stats(self) -> str:
        by = {"DRAFT": 0, "EVALUATING": 0, "REVIEWED": 0, "CHALLENGE_WINDOW": 0, "APPEALED": 0, "GATED": 0, "ARCHIVED": 0}
        supported = 0
        contradicted = 0
        unclear = 0
        i = 0
        while i < len(self.cases):
            ...
            by[case.get("status", "DRAFT")] = by.get(case.get("status", "DRAFT"), 0) + 1
            if case.get("outcome") == "approved":
                supported += 1
            elif case.get("outcome") == "blocked":
                contradicted += 1
            elif case.get("outcome") == "conditional":
                unclear += 1
            i += 1
        return json.dumps({"contract": "EvalGate", "cases": len(self.cases), "evidence": len(self.evidence),
                           ...
                           "approved": supported, "blocked": contradicted, "conditional": unclear})
    ...
    def get_quality_score(self) -> str:
        ...
        return json.dumps({"qualityBps": q, "reason": "model_release, evaluation, risk_finding, GenLayer review, challenge, appeal and audit coverage"})

    @gl.public.view
    def get_frontend_bootstrap(self) -> str:
        return json.dumps({"contract": "EvalGate", "statuses": list(STATUSES), "outcomes": list(OUTCOMES),
                           "legacyNoun": "model_release", "product": "AI model evaluation and release decision", "childTypes": ["evaluation", "risk_finding"], "recentCases": json.loads(self.get_recent_cases(10)),
    ...
    @gl.public.write
    def propose_model_release(self, title: str, modelFamily: str, claim: str, source_url: str) -> int:
        fields = {"modelFamily": _s(modelFamily, 320), "domain": "model releases, model cards, evaluation suites, benchmark runs, risk findings and release gates"}
        scoped_claim = _s(claim, 1800) + " | Model family: " + _s(modelFamily, 320)
        return int(self._create_case("model_release", title, scoped_claim, source_url, fields))

    @gl.public.write
    def assess_release_gate_with_genlayer(self, item_id: int) -> None:
        self.review_with_genlayer(str(item_id))

    @gl.public.write
    def archive_model_release(self, item_id: int) -> None:
        self.archive_case(str(item_id))

    @gl.public.view
    def get_model_release_count(self) -> int:
        return len(self.cases)

    @gl.public.view
    def get_model_release(self, item_id: int) -> dict:
        if item_id < 0 or item_id >= len(self.cases):
            return {}
        case = json.loads(self.cases[item_id])
        return {"id": item_id, "model_release": case["title"], "claim": case["claim"],
                "modelFamily": case.get("fields", {}).get("modelFamily", ""),
                ...
                "evaluations": len(self._ilist(self.idx_case_evaluations, case["id"])),
                "risk_findings": len(self._ilist(self.idx_case_risk_findings, case["id"]))}

    @gl.public.view
    def get_stats(self) -> dict:
        return {"total": len(self.cases),
                "gated": len(self._ilist(self.idx_status, "GATED")),
                "draft": len(self._ilist(self.idx_status, "DRAFT")),
                "evaluations": len(self.evaluations),
                "risk_findings": len(self.risk_findings)}
