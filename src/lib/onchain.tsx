...
export function OnchainProvider({ children, initialSnapshot = null }: { children: ReactNode; initialSnapshot?: OnchainSnapshot | null }) {
  ...
  const execute = useCallback(async (functionName: string, args: unknown[]) => {
    ...
    setLastTx(hash);
    await client.waitForTransactionReceipt({ hash, status: 'GATED' as never, retries: 200 });
  ...
  return (
    <OnchainContext.Provider value={contextValue}>
      ...
      {dockOpen && (
        <div className="chain-dock-scrim chain-dock-scrim--eval-gate" role="presentation" onMouseDown={event => {
          if (event.currentTarget === event.target) setDockOpen(false);
        }}>
          <section className="chain-dock chain-dock--eval-gate" data-panel="eval-gate" role="dialog" aria-modal="true" aria-label="Onchain contract controls">
            <header>
              <div><span>MODEL RELEASE LAB</span><b>{snapshot.contract}</b></div>
            ...
            {mode === 'create' && (
              <div className="chain-fields">
                <label>Title<input value={title} onChange={event => setTitle(event.target.value)} placeholder="Model release" /></label>
                <label>Claim<textarea value={claim} onChange={event => setClaim(event.target.value)} placeholder="NIST AI risk management resources support documenting measurement, evaluation, risk treatment and governance before an AI system release." /></label>
                <label>Public source URL<input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://..." /></label>
                <button type="button" disabled={Boolean(busy) || !title.trim() || !claim.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('propose_model_release', [title.trim(), "Model family", claim.trim(), sourceUrl.trim()])}>
                  {busy === 'propose_model_release' ? 'Waiting for finality' : 'Propose release onchain'}
                </button>
              </div>
            )}
            ...
            {mode === 'evidence' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || !operator || !title.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('add_evidence', [selected.id, sourceUrl.trim(), title.trim(), note.trim()])}>
                  {busy === 'add_evidence' ? 'Waiting for finality' : operator ? 'Add evidence onchain' : 'Model_release operator required'}
                </button>
              </div>
            )}
            ...
            {mode === 'review' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || !operator || selected.status !== 'DRAFT' || selected.evidenceCount === 0} onClick={() => void run('review_with_genlayer', [selected.id])}>
                  {busy === 'review_with_genlayer' ? 'Validators are reviewing' : !operator ? 'Model_release operator required' : selected.status !== 'DRAFT' ? `Record is ${selected.status}` : 'Run validator review'}
                </button>
              </div>
            )}
            ...
            {mode === 'challenge' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || selected.status !== 'CHALLENGE_WINDOW' || !claim.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('submit_challenge', [selected.id, claim.trim(), sourceUrl.trim()])}>
                  {busy === 'submit_challenge' ? 'Waiting for finality' : selected.status !== 'CHALLENGE_WINDOW' ? `Record is ${selected.status}` : 'Submit challenge onchain'}
                </button>
            ...
            {mode === 'appeal' && selected && (
              <div className="chain-fields">
                ...
                <label>Public evidence URL<input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://..." /></label>
                <button type="button" disabled={Boolean(busy) || !['REVIEWED', 'CHALLENGE_WINDOW'].includes(selected.status) || !note.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('submit_appeal', [selected.id, note.trim(), sourceUrl.trim()])}>
                  {busy === 'submit_appeal' ? 'Waiting for finality' : !['REVIEWED', 'CHALLENGE_WINDOW'].includes(selected.status) ? `Record is ${selected.status}` : 'Submit appeal onchain'}
                </button>
            ...
            {mode === 'lifecycle' && selected && (
              <div className="chain-fields chain-lifecycle">
                ...
                <button type="button" disabled={Boolean(busy) || !operator || !['REVIEWED', 'CHALLENGE_WINDOW'].includes(selected.status)} onClick={() => void run('finalize_case', [selected.id])}>Finalize eligible record</button>
                <button type="button" disabled={Boolean(busy) || !operator || selected.status !== 'GATED'} onClick={() => void run('archive_case', [selected.id])}>Archive finalized record</button>
