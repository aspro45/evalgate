...
test("live Studionet record exposes the reviewed model release workflow", { timeout: 180_000 }, async () => {
  ...
  assert.equal(record.status, "CHALLENGE_WINDOW");
  assert.ok(["approved","blocked","conditional"].includes(String(record.outcome)));
  const stats = parse(await read("get_contract_stats"), {});
  assert.ok(Number(stats.cases) >= 1);
  assert.ok(Number(stats.evidence) >= 3);
  assert.ok(parse(await read("get_evaluations", [id]), []).length >= 1);
  assert.ok(parse(await read("get_risk_findings", [id]), []).length >= 1);
